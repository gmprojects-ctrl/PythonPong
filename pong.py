...
def WriteText(font,screen,text):
    ...
    position=(int((width/2)-(font.size(text)[0]/2)), #Allow the the text to be centered on the screen
...
#Making the classes
class Block:
    def __init__(self,block_width,block_height,x,y,v): #Initialising the size, position and velocity of each block
    ...
    def move(self): # Alloscreeng the player character to move
        ...
        if keys[pygame.K_UP] and (self.y > 0):
            ...
            self.y+=self.v
    def emove(self,ball): # Allow the bot character to move based on the position of the ball
        if (ball.x>= width/2):
            if (ball.y_v>0):
                self.y+=self.v
            elif(ball.y_v<0):
...
class Ball: 
    ...
    def outbounds(self,player,enemy):
        ...
        if (self.x <= (0+self.r)) or (self.x >= (width-self.r)):
            ...
            self.y=int(height/2)
            self.x_v*=-1
            WriteText(font,screen,f"Player {player.points} | Bot {enemy.points}")
...
enemy=Block(20,100,width-player.x,player.y,5)
winscore=5
#Main game
state= True
...
while state:
    ...
    if (player.points >= winscore): # If the player or bot wins, shows it and refreshes the game)
        player.points = 0
        enemy.points = 0
